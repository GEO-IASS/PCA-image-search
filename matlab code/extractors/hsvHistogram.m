...
for row = 1:size(h, 1)
    for col = 1 : size(h, 2)
        ...
        index(count, 1) = ceil(numberOfLevelsForH * h(row, col)/maxValueForH);
        index(count, 2) = ceil(numberOfLevelsForS * s(row, col)/maxValueForS);
        index(count, 3) = ceil(numberOfLevelsForV * v(row, col)/maxValueForV);
        count = count+1;
    end
end
